...
{
	BOOL bPass;
	DWORD readVal1 = 0x03FF5C00;
	DWORD readVal2 = 0x5C004614;
	DWORD readVal3 = 0x0A0A8000

	bPass = PCIE_Write32( hPCIe, pcie_bars[0], addr, readVal1);
		if (!bPass)
		{
		...
		addr = addr + 4;
	bPass = PCIE_Write32( hPCIe, pcie_bars[0], addr, readVal2);
		if (!bPass)
		{
			printf("test FAILED: write did not return success");
			return;
		...
		addr = addr + 4;
	bPass = PCIE_Write32( hPCIe, pcie_bars[0], addr, readVal3);
		if (!bPass)
		{
			printf("test FAILED: write did not return success");
			return;
